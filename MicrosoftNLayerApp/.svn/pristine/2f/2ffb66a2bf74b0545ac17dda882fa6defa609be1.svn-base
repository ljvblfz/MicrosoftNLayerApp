...
namespace Microsoft.Samples.NLayerApp.Application.MainModule.SalesManagement
{
        ...
        : ISalesManagementService
    {
        ...
        /// <param name="cacheManager">Cache manager dependency, this dependency is resolved with IoC</param>
        public SalesManagementService(IOrderRepository orderRepository, IProductRepository productRepository,ICacheManager cacheManager)
        {
        ...
        public List<Product> FindPagedProducts(int pageIndex, int pageCount)
        {
            ...
            CacheKey key = new CacheKey("FindPagedProducts", new { PageIndex = pageIndex, PageCount = pageCount });
            CacheItemConfig cacheItemConfig = new CacheItemConfig(key, new TimeSpan(0, 0, 10));

            if (_cacheManager.TryGet<List<Product>>(cacheItemConfig, out productResult))
                return productResult;
            else
            {
                //query repository
                productResult =  _productRepository.GetPagedElements(pageIndex, pageCount, p => p.ProductId, true)
                                                   .ToList();

                _cacheManager.Add(cacheItemConfig, productResult);
